...
console.log("==== NC PHASE 5 FINAL QA ====\n");
...
const ncResIds = new Set(all.map(r=>r.id));
const allJunc = await paginate("resource_categories", "resource_id, category_id");
const ncJunc = allJunc.filter(j => ncResIds.has(j.resource_id));
const orphanRes = ncJunc.filter(j => !globalResIds.has(j.resource_id));
const orphanCat = ncJunc.filter(j => !globalCatIds.has(j.category_id));
...
const badLatLng = approved.filter(r => r.latitude && (r.latitude < 33.7 || r.latitude > 36.7 || r.longitude > -75.3 || r.longitude < -84.5));
console.log(`--- Geometry: city-but-no-coords=${noLatLng.length}  outside-NC-bbox=${badLatLng.length} ---`);
...
//     use the GLOBAL orphan signal, not the NC-scoped one which is tautological)
...
if (orphanCat.length > 0) fails.push(`${orphanCat.length} NC orphan-cat junctions`);
if (bleed.length > 0) fails.push(`${bleed.length} wrong-state bleed`);
if (badLatLng.length > 0) fails.push(`${badLatLng.length} outside-NC-bbox`);
...
// NC Phase 5 floor: target was ALL 17 cats ≥30; report progress without hard-failing
// (SC Phase 6 left disabled-veterans/family-support/substance-recovery/transportation under floor too)
const weakAfter = cats.filter(c => (catCount[c.id]||0) < FLOOR).map(c => `${c.slug}=${catCount[c.id]||0}`);
console.log(`  Phase 5 weak-cat report (info, not hard-fail): ${weakAfter.length === 0 ? "ALL CATS ≥30" : weakAfter.join(", ")}`);
console.log(`\n=== FINAL VERDICT ===`);
...
console.log(`  Note: NC-scoped orphan-res check is tautological (ncJunc is built FROM ncResIds);`);
